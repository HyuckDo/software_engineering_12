...
<head>
	...
	function checkId(){
		var id = document.getElementById("f").admin_id.value
		if(!id){
			alert("아이디를 입력하세요");
			return false;
	...
	function sendCheck(){
		if(document.getElementById("f").checkcheck.value=="check"){
			...
			opener.document.f.admin_id.value = document.getElementById("admin_id").value;
...
<body>
	...
	<hr>
	<br>
		<form id="f" action="checkIdP.mo" method="post">
			아이디<input type="text" name="admin_id" id="admin_id" value="${param.admin_id}" onkeydown="resetcheck()"/> <input type="button" value="중복체크" onclick="checkId()"/>
